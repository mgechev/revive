...
- `allowed-packages`: (list of strings) list/array of allowed dot import packages
...
- `convention-name-check-regex`: (string) Custom regex pattern to validate package names.
  If set, package names must match this pattern.
...
- `check-extra-bad-name`: (bool) If `true`, enable checks for extra bad package names (e.g., `helpers`, `models`, `shared`, `utilities`).
  Default: `false`.
...
_Configuration_: Supports a single `map[string]any` argument with an `allow-regex` option to specify additional allowed patterns for
unused parameter names beyond the default `_`.
