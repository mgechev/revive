...
- `allowed-packages`: (list of strings) comma-separated list of allowed dot import packages
...
- `say-repetitive-instead-of-stutters` replaces the use of the term _stutters_
...
- `skip-convention-name-check`: (bool)
...
- `convention-name-check-regex`: (string)
...
_Configuration_: Supports arguments with single of `map[string]any` with option `allow-regex` to provide additional
...
_Configuration_:
Supports arguments with single of `map[string]any` with option `allow-regex` to provide additional to `_` mask to allowed unused receiver names.
