...
_Description_: Sometimes `gofmt` is not enough to enforce a common formating of a code-base, this rule warns when there are heading or trailing newlines in code blocks.
...
and may be a symptom of bad design.
...
_Description_: A method that modifies its receiver value can be hard to understand.
...
`append`, `make`, and basic type names like `bool`, and `byte` are not reserved 
words of the language, therefore the can be redefined. 
