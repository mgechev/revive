...
func useSlicesConcat(s1, s2, s3 []int) {
	...
	switch len(s1) {
	case 0:
		inCase := append([]int{}, s1...) // MATCH /replace consecutive appends by a call to slices.Concat/
	...
	select {
	case <-done:
		inComm := append([]int{}, s1...) // MATCH /replace consecutive appends by a call to slices.Concat/
